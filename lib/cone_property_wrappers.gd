...
# TODO better newline, and add it in more places
#! <List> </List>
...
#! We represent it as a vector where the first entry is the rank. It is followed by sequence of pairs of entriey n,m. Such two entires represent a free cyclic summand (Z/nZ)^m.
...
#! given as list of polynomials P_0, ..., P_(p-1) such that Q(i) = P_(i%p) (i).
...
#! scheme. Given as a list pairs. The first pair entry is a key of generators contained in the face (compare also <Ref Func="NmzTriangulation"/>) and the multiplicity with which it was considered.
